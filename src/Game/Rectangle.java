...
public class Rectangle {
     ...
     */
        ...
        // TODO: 1. Triển khai phần code kiểm tra va chạm giữa 2 hình chữ nhật ở đây
            if ( Math.abs((this.position.x + this.width/2) - (other.position.x + other.width/2)) < Math.abs((this.width + other.width)/2) &&
                 (Math.abs((this.position.y + this.height/2) - (other.position.y + other.height/2)) < Math.abs((this.height + other.height)/2))) {
            return true;
            }
            return false;
            }
